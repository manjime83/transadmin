...
aws cloudformation package --template-file cloudformation/template.yaml --s3-bucket $PROJECT_NAME-devops --s3-prefix $ENV --output-template-file .aws/template.yaml
...
aws cloudformation deploy --template-file .aws/template.yaml --stack-name $PROJECT_NAME-$ENV --capabilities CAPABILITY_NAMED_IAM --no-fail-on-empty-changeset \
    --parameter-overrides ProjectName=$PROJECT_NAME Environment=$ENV $(cat .env.$ENV | tr -s [:space:] ' ')
